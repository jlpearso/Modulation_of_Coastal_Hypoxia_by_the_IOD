...
%
% >> N = 5;
% >> rgb = maxdistcolor(N,@sRGB_to_Lab) % CIELAB colorspace
% rgb =
%          0         0    1.0000
%          0    1.0000         0
%     1.0000    0.4331         0
%     1.0000    0.0709    0.6508
%          0         0         0
...
%
%%% The following examples use CIECAM02 colorspace, downloaded separately.
%
% >> maxdistcolor(5,@sRGB_to_Jab, 'exc',[0,0,0]) % Exclude black background.
% ans =
%     1.0000    1.0000         0
%     1.0000    0.7323    1.0000
%          0    0.4803         0
%     0.7143         0    0.0159
%     0.0159    0.3071    1.0000
%
% >> maxdistcolor(5,@sRGB_to_Jab, 'inc',[1,0,1]) % Include magenta.
% ans =
...
bgt = now();
%
assert(isnumeric(N)&&isscalar(N),'SC:maxdistcolor:NotNumericScalarN',...
	'First input <N> must be a numeric scalar.')
assert(isfinite(N)&&N>=0,'SC:maxdistcolor:NotPositiveFiniteN',...
	'First input <N> must be a finite positive value. Input: %g',N)
assert(isreal(N),'SC:maxdistcolor:IsComplexN',...
	'First input <N> cannot be a complex value. Input: %g%+gi',N,imag(N))
%
assert(isa(fun,'function_handle'),'SC:maxdistcolor:NotFunctionHandle',...
	'Second input <fun> must be a function handle.')
map = fun([0,0,0;1,1,1]);
assert(isfloat(map),'SC:maxdistcolor:FunOutNotNumericArray',...
	'Second input <fun> must return a floating-point matrix.')
assert(isequal(size(map),[2,3]),'SC:maxdistcolor:FunOutInvalidSize',...
	'Second input <fun> output matrix has an incorrect size.')
assert(all(isfinite(map(:))),'SC:maxdistcolor:FunOutNotFiniteValue',...
	'Second input <fun> output matrix values must be finite.')
assert(isreal(map),'SC:maxdistcolor:FunOutIsComplex',...
...
if nargin==3
	assert(isstruct(opts)&&isscalar(opts),'SC:maxdistcolor:NotScalarStruct',...
...
elseif nargin>3 % options as <name-value> pairs
	...
	assert(isscalar(opts),'SC:maxdistcolor:CellArrayValue',...
...
stpo = opts;
%
assert(stpo.Lmax>stpo.Lmin,'SC:maxdistcolor:LmaxLessThanLmin',...
	'The value Lmax must be greater than the value Lmin.')
assert(stpo.Cmax>stpo.Cmin,'SC:maxdistcolor:CmaxLessThanCmin',...
...
%
assert(size(intersect(exc,inc,'rows'),1)==0,'SC:maxdistcolor:ExcIncSameColors',...
...
case 'zip'
	...
	ids([1:2:N,2:2:N]) = ids;
otherwise
	error('SC:maxdistcolor:UnknownSortOption',...
...
if any(idf)
	...
	dfn = sprintf(' ''%s'',',dfc{:});
	error('SC:maxdistcolor:UnknownOption',...
...
%
	...
	if any(idx)
		map = uvc{idx};
		S = size(map);
		assert(isnumeric(map),'SC:maxdistcolor:NotNumericRGB',...
			'The <%s> input must be a numeric matrix.',fld)
		assert(numel(S)==2,'SC:maxdistcolor:NotMatrixRGB',...
			'The <%s> input must be a numeric matrix ',fld)
		assert(S(2)==3||all(S==0),'SC:maxdistcolor:InvalidSizeRGB',...
			'The <%s> input must be empty or have size Nx3.',fld)
		assert(all(isfinite(map(:))),'SC:maxdistcolor:NotFiniteValueRGB',...
			'The <%s> RGB values must be finite.',fld)
		assert(isreal(map),'SC:maxdistcolor:ComplexValueRGB',...
	...
	if any(idx)
		val = uvc{idx};
		assert(isnumeric(val),'SC:maxdistcolor:NotNumericValue',...
			'The <%s> input must be numeric. Class: %s',fld,class(val))
		assert(isscalar(val),'SC:maxdistcolor:NotScalarArray',...
			'The <%s> input must be scalar. Numel: %d',fld,numel(val))
		assert(imag(val)==0,'SC:maxdistcolor:ScalarIsComplex',...
			'The <%s> value cannot be complex. Input: %g%+gi',fld,val,imag(val))
		assert(isf||(fix(val)==val),'SC:maxdistcolor:ScalarIsNotInteger',...
			'The <%s> value must be integer. Input: %g',fld,val)
		assert(val>=minV,'SC:maxdistcolor:ScalarValueIsAboveRange',...
			'The <%s> value must be >=%g. Input: %g',fld,minV,val)
		assert(val<=maxV,'SC:maxdistcolor:ScalarValueIsBelowRange',...
	...
	if any(idx)
		...
		if ~(ischar(tmp)&&isrow(tmp)&&any(strcmpi(tmp,varargin)))
			error('The <%s> value must be one of:%s',fld,sprintf(' ''%s'',',varargin{:}));
		end
		stpo.(fld) = lower(tmp);
	end
	end
...
%
assert(N<10,'SC:maxdistcolor:TooManyPermutations',...
