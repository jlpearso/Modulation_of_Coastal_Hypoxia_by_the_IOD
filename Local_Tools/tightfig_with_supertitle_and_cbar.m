function hfig = tightfig_with_supertitle_and_cbar(hfig,offset)
...
%         set(hax(i), 'LooseInset', [0,0,0,0]);
    ...
    for i = 1:numel(hax)
        ...
        set(hax(i), 'Position', [pos(i,1:2) - [moveleft,movedown], pos(i,3:4) + [0,offset]]);
        
    end
    
    for i = 1:numel(hcbar)
        
        set(hcbar(i), 'Position', [pos(i+numel(hax),1:2) - [moveleft,movedown], pos(i+numel(hax),3:4) + [0,offset]]);
        
    end
    
    for i = 1:numel(hleg)
        
        set(hleg(i), 'Position', [pos(i+numel(hax)+numel(hcbar),1:2) - [moveleft,movedown], pos(i+numel(hax)+numel(hcbar),3:4)+ [0,offset]]);
        ...
        if strcmp (hcbar.AxisLocation, 'out')
            ...
            switch hcbar.Location
                ...
                case 'northoutside'
                    ...
                    ticklablespace_cm = 1.75 * (0.3528/10) * fsize;
                ...
                case 'eastoutside'
                    ...
                    ticklablespace_cm = (0.3528/10) * fsize * maxlabellen * 1.75;
